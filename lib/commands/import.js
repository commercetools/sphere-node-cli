...
import CustomObjectImporter from '@commercetools/custom-objects-importer'
...
const allowedTypes = pick(
  ...
  'category', 'customer', 'discount', 'order', 'discountCode',
  'state', 'customObject',
)
...
export default class ImportCommand extends Command {
  run (argv) {
    ...
    if (!isTest && argv.length <= 3) this.program.help()

    const options = Object.assign({}, this.program)
    if (!options.plugin) this._validateOptions(options, 'type')

    return this._preProcess(options)
  }

  _process (options) {
    if (options.plugin) return this._processWithPlugin(options)

    const service = serviceMapping[options.type]
    if (service) return this._stream(...service(options))
  ...
  _stream (options, jsonPath, processFn, finishFn) {
    let inputStream
    if (options.from) inputStream = fs
      .createReadStream(options.from, { encoding: 'utf-8' })
    else {
    ...
    return ___(transformStream).reduce(null, () => '')
      .stopOnError(e => this
        ._die('Something went wrong while transforming chunks.\n', e))
